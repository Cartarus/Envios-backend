...
/**
 * Ejecuta las migraciones en el pool proporcionado
 * Compatible con pg-mem para tests y PostgreSQL real para desarrollo
 */
export async function runMigrations(pool: Pool, skipSeed = false) {
  try {
    // Ejecutar cada migración en orden
    for (const migration of migrations) {
      const migrationPath = join(__dirname, '../infrastructure/migrations', migration);
      let migrationSQL = readFileSync(migrationPath, 'utf-8');
      
      // pg-mem no soporta UUID, convertir a VARCHAR
      migrationSQL = migrationSQL.replace(/UUID/g, 'VARCHAR(36)');
      
      await pool.query(migrationSQL);
    }
    
    // Poblar con datos de prueba si se requiere
    if (!skipSeed) {
      await seedData(pool);
    }
  } catch (error) {
